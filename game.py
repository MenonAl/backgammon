...
def game():
    global map_stacs, white_checkers, black_checkers, dice, move, double, checkers, point, checkers_quantity
    if str(checkers) == player:
        ...
        if not map_stacs[int(point) - 1]:
            ...
            return "Good" + render_template('move.html', size=len(map_stacs), checkers_quantity=checkers_quantity)
    else:
...
def game_move():
    global map_stacs, white_checkers, black_checkers, dice, move, double, action, checkers, point, checkers_quantity
        ...
        if map_stacs[point - 1 + dice_move][0] != str(checkers) and map_stacs[point - 1 + dice_move][0] != "-1":
            ...
            for i in range(0, checkers_quantity):
                if map_stacs[point - 1][i] == "-1":
                    break
            print(map_stacs[point - 1])
            map_stacs[point - 1].pop(i)
